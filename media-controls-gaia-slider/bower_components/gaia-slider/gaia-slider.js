...
module.exports = component.register('gaia-slider', {
  created: function() {
    ...
    this.els.input.addEventListener('change', this.onChanged.bind(this));
    this.updateOutput();
  },

  onChange: function(e) {
    if (this.moving) {
      this.moving(this.els.input.value);
    }

    console.log('firing moving event');
    this.dispatchEvent(new CustomEvent('slider-moving',
  ...
  onChanged: function(e) {
    ...
    console.log('firing moved event');
    if (this.moved) {
      this.moved(this.els.input.value);
    }
    this.dispatchEvent(new CustomEvent('slider-moved',
      { 'detail': this.els.input.value }));
    this.updateOutput();
  ...
  max: function(value) {
    ...
    if (value === this.els.input.max || isNaN(value)) { return; }
    this.els.input.max = value;
    console.log('setting max to ' + value);
  },

  value: function(value) {
    value = parseInt(value, 10);
    if (value === this.els.input.value || isNaN(value)) { return; }
    this.els.input.value = value;
    console.log('setting value to ' + value);
  },

  registerMovementCallbacks: function(moving, moved) {
    this.moving = moving;
    this.moved = moved;
  },
